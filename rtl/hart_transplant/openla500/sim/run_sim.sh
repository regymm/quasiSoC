...
echo -e "\033[37mPreparing boot loader firmware...\033[0m"
docker run -it --rm -v ../../../..:/mnt regymm/rv32ima-gocd:latest make -C /mnt/firmware
#make -C ../firmware
...
if [[ "$1" == "verilator" ]]; then
    ...
    verilator -j 0 -O3 --x-assign fast --x-initial fast --noassert -DSIMULATION -DINTERACTIVE_SIM --top-module quasi_main_sim \
         ...
         -Wno-timescalemod \
         -Wno-realcvt \
         -GSIMULATION=1 -GINTERACTIVE_SIM=1 -GCLOCK_FREQ=33000000 -GBAUD_RATE_UART=115200 -DBRAM_EN -DNO_SDRAM \
    ...
    #vvp -n a.out
    obj_dir/Vquasi_main_sim 100000000 # --trace --sdcard=sdcard.img # $timeout

    echo -e "\033[37mFinished.\033[0m"
elif [[ "$1" == "iverilog" ]]; then
